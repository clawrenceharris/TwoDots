...
public class InputService : MonoBehaviour
{
    ...
    const float maxHitRadius = 0.5f; // tune based on cell size / sprite size
    ...
    for (int dx = -1; dx <= 1; dx++)
    {
            for (int dy = -1; dy <= 1; dy++)
            {
                ...
                if (candidateWorld.x <= worldPosition.x + maxHitRadius && candidateWorld.x >= worldPosition.x - maxHitRadius && candidateWorld.y <= worldPosition.y + maxHitRadius && candidateWorld.y >= worldPosition.y - maxHitRadius)
                {
                    bestDot = candidate;
                }
            }
